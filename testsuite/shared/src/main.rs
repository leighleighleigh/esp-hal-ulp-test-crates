...
fn get_global_symbols(
    elf_file: &mut ElfStream<AnyEndian, std::fs::File>,
    allow_zero_sized: bool,
) -> Vec<GlobalObjectSymbol> {
    ...
    symtab
        ...
        .map(|sym| {
            ...
            let _sym_section = elf_section_hdrs[sym.st_shndx as usize];
            let symbol_address = sym.st_value;
            GlobalObjectSymbol {
...
fn print_global_symbols(
    ...
    // }
    table.set_header(["sym_addr", "sym_name"]);
...
fn print_global_symbols_as_linkerscript(
    elf_file: &mut ElfStream<AnyEndian, std::fs::File>,
    base_address: u64,
    allow_zero_sized: bool,
) {
    ...
    for sym in global_syms.iter() {
        println!(
            "PROVIDE({} = {});",
            sym.name.clone(),
            format!("{:#x}", sym.address + base_address)
        );

...
use esp_hal::{ulp_core::{UlpCore,UlpCoreWakeupSource},gpio::rtc_io::{LowPowerOutput,LowPowerInput,LowPowerOutputOpenDrain}};
...
fn main() -> ! {
    ...
    #[cfg(feature = "ulp-app-stompy")]
    {
        // Install ulp-apps/esp32s3-stompy-ulp-lp-core-ws2812b
        #[cfg(any(esp32s3))]
        let ulp_core_code = load_lp_code!("./ulp-apps/esp32s3-stompy-ulp-lp-core-ws2812b");
    ...
    let data = (0x5000_1000) as *mut u32;

    loop {
        // Print the debugging information
        // let ulpdbg = read_coproc_debug();
        // info!("{:?}", &ulpdbg);
        // dump_coproc_pc_instructions(ulpdbg);
        info!("ULP counter {}", unsafe {data.read_volatile()});
